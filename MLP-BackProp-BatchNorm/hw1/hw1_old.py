...
class Sigmoid(Activation):
    ...
    def derivative(self):
        ...
        forward_x = self.forward(self.state)
...
class BatchNorm(object):
    ...
    def backward(self, delta):
        ...
        m = self.x.shape[0]
        dLdXci = np.multiply(self.norm,self.gamma)  
        self.dbeta = np.sum(delta, axis = 0)
        self.dgamma = np.sum(np.multiply(delta, self.norm), axis = 0)
        dXcidVar = 0.5 * np.multiply((self.mean - self.x) , ((self.var + self.eps)**-1.5))
        
        dLdMu = ((-1 * np.multiply(dLdXci, ((self.var + self.eps)**-0.5))) + \
                  np.multiply((-1 * (2/m) * dXcidVar) , np.sum((self.x - self.mean), axis = 0)))
        
        dLdXi = (np.multiply(dLdXci, ((self.var + self.eps)**-0.5)) + \
                np.multiply(dXcidVar, ((2/m) * (self.x - self.mean))) + \
                (dLdMu * (1/m)))
